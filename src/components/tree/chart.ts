...
export default class Chart {
  ...
  initData() {
    ...
    this.clickNode = Object.assign(this.treeMap(this.root), { x0: 0, y0: 0 });
  ...
  template() {
    ...
    const svg = d3
      ...
      .attr("viewBox", [-width / 2, -height / 2, width, height - 63].join(',')) // 设置视窗大小
  ...
  mouseOver(d: HierarchyPointNode) {
    ...
    if (showDesc) {
      ...
      ][] = [["id", "", "**$**"], ["version", "版本"], ["dir", "目录"],
        ["requiring", "依赖", n => "**" + n.requiring.length + "**个包"],
        ["requiredBy", "应用", n => "**" + n.requiredBy.length + "**个包"],
        ["depth", "", depth]
      ];
      inner.join("p")
        .html((n, i, a) => list
